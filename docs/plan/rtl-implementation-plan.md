...
| Determinism | seedrandom (JS) + numpy Generator (Python) | One seed threaded everywhere |
...
- [ ] `grep -r "Math.random()" packages/ sim/src/` returns only `rng.ts`
- [ ] ESLint fails on a deliberate `Math.random()` in a package
...
**Key Logic:**
- CI jobs: `lint` → `test-js` → `test-py` → `licence` → `sbom` → `build-sim` → `e2e-smoke`
- SBOM generated with CycloneDX for both ecosystems, uploaded as a build artifact on every run (vendor review will ask for it)
- Pin all GitHub Actions to a commit SHA, not a tag
- Release workflow is tag-triggered and fully automated: version bump → wheel → container → GHCR push → GitHub Release
...
- `bench/rtlenv/domain_protocol.py` (Python mirror)
...
- A domain declares: `entities` (typed records), `operations` (name, params, preconditions, effects), `invariants` (predicates that must hold after every operation), and `seedState(rng)`.
- State is one JSON-serialisable tree. No hidden state, no closures, no Dates captured at import time.
- `diff(before, after)` returns a structured changeset, not a string.
